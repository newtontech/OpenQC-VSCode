...
suite('Extension Test Suite', () => {
  ...
  test('Command: openqc.plotData should be registered', async () => {
    const commands = await vscode.commands.getCommands(true);
    assert.ok(commands.includes('openqc.plotData'), 'openqc.plotData command should be registered');
  });
  ...
  test('Command: openqc.startLSP should be registered', async () => {
    const commands = await vscode.commands.getCommands(true);
    assert.ok(commands.includes('openqc.startLSP'), 'openqc.startLSP command should be registered');
  });

  test('Command: openqc.stopLSP should be registered', async () => {
    const commands = await vscode.commands.getCommands(true);
    assert.ok(commands.includes('openqc.stopLSP'), 'openqc.stopLSP command should be registered');
  });
  ...
  test('Command: openqc.validate should be registered', async () => {
    const commands = await vscode.commands.getCommands(true);
    assert.ok(commands.includes('openqc.validate'), 'openqc.validate command should be registered');
  });
