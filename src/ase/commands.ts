    ...
    await vscode.window.withProgress(
        ...
        async (progress) => {
            ...
            const formula = result.atoms?.chemical_symbols ?
                getChemicalFormula(result.atoms.chemical_symbols) : 'Unknown';
...
async function migrateFormat(converter: ASEConverter): Promise<void> {
    ...
    const suggestedOutput = `${inputBasename}_${selected.format}${formats[selected.format].extensions[0]}`;
...
async function quickConvert(converter: ASEConverter): Promise<void> {
    ...
    await vscode.window.withProgress(
        ...
        async (progress) => {
            ...
            vscode.window.showInformationMessage(
                `Successfully converted: ${outputPath}`
            );
        }
    );
}
