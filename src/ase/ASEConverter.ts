...
export class ASEConverter {
    ...
    public async isAvailable(): Promise<boolean> {
        try {
            const { stdout } = await execAsync(`${this.pythonPath} -c "import ase; print(ase.__version__)"`);
     ...
     */
    public getSupportedFormats(): Record<string, { name: string; extensions: string[]; description: string }> {
        return {
            vasp: {
                name: 'VASP',
